...
let startYear = paymentDate.getMonth() >= 5 
  ? paymentDate.getFullYear() 
  : paymentDate.getFullYear() - 1;

const validFrom = new Date(startYear, 5, 1);     // June 1st
const validTo   = new Date(startYear + 1, 5, 30); // June 30th
  ...
  validFrom: validFrom.toISOString().split('T')[0], // e.g. "2025-06-01"
  validTo: validTo.toISOString().split('T')[0],     // e.g. "2026-05-31"
  month: 6,                                         // Always May (valid till month)
  year: validTo.getFullYear()                       // Dynamic year (2026, 2027…)
};
await UserService.addBusPass(pass);
setBusPass(pass);

        // Refetch student data from Firestore to update payment status immediately
...
async function generateBusPassForUser() {
  try {
    ...
    // If current month >= June → start this year, else previous year
    let startYear = paymentDate.getMonth() >= 5 
      ? paymentDate.getFullYear() 
      : paymentDate.getFullYear() - 1;
const validFrom = new Date(startYear, 5, 1);     // June 1st
const validTo   = new Date(startYear + 1, 5, 30); // June 30th
    ...
    console.error("Error generating bus pass:", err);
  }
}
  ...
  <p><strong>4. Contact</strong><br />For queries or payment-related issues, please contact the TCE Transport Office or email: <a href="mailto:transport@gen.tce.edu" className="text-blue-600 underline">transport@gen.tce.edu</a></p>
                        ...
                        <div className="text-sm text-gray-700 mb-6 space-y-2 max-h-48 overflow-y-auto border p-3 rounded">
                          <p><strong>1. Nature of Service</strong><br />This App does not facilitate physical goods delivery. Instead, it processes digital service transactions related to student bus fare payments.</p>
                ...
                <div className="mt-6 bg-green-50 border border-green-200 rounded-lg p-3">
                  ...
                  {busPass && (
                    <div className="mt-4 p-4 bg-white border border-green-300 rounded-lg shadow">
                      ...
                      <div className="mt-2 text-xs text-gray-500">Pass will be removed after this year.</div>
...
<button
  ...
  onClick={async () => {
    ...
    pdf.setFontSize(8);
    pdf.text("Madurai , Tamil Nadu – 625 015", width / 2 + 12, margin + 20, { align: "center" });
...
// If month is Jan–May → valid till June of THIS year
// If month is June–Dec → valid till June of NEXT year
if (now.getMonth() <= 4) {   // 0=Jan, 4=May
  validYear = now.getFullYear();
} else {
  validYear = now.getFullYear() + 1;
}

pdf.text(`Valid Till: June ${validYear}`, margin + 10, textY);
textY += 12;
    ...
    const photoX = width - margin - photoWidth - 6;
...
await QRCodeLib.toCanvas(qrCanvas, qrValue, {
  width: 100, // increase size for clarity
  margin: 1,
});
...
pdf.addImage(qrDataUrl, "PNG", width - 60, height - 60, 50, 50);
